...
\begin{enumerate}
	...
	$$ n_j = \{i \in I ~|~ a_{ij} \neq 0\} $$ 
	The hypergraph of the example $5 \times 4$ matrix is $H=(\{1,\ldots,5\},\{\{4\},\{0, 3\},\{1,4\},\{2\}\})$ and it's graphical representation can be found in \autoref{fig:hypergraph}.
	...
	\end{figure}
	To find the distribution model we still have to partition $H$ over the available processors. Since we are working with a column-net model coarsening means combining similar rows. For this example we assume that there are 2 processors available so we execute a 2-way multi-level partition.
	\begin{equation}
	\begin{multlined}
		A_{5\times4} = 
		\begin{bmatrix}
	...
	\end{multlined}
	\end{equation}
	We found the following 2-way row partition: $P_2 = \{\{0,2,3\},\{1,4\}\}$. No refinements were needed.
	\item The equation
		\begin{equation}

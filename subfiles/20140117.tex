...
\begin{solution}
\begin{enumerate}
		\item The sequential cost of $y=Ax$ is $2cm$ flops \cite[p.166]{bisseling04} where $c$ is the fixed number of nonzeroes in a row of A and $m$ the amount of rows. This holds because the usded data-structures make it possible to only take multiplications with a nonzero into account.
		\item
\end{enumerate}
\end{solution}

		...
		\item "A partitioning of a column-net model of A corresponds to a partitioning of the rows of A (a 1D row-wise distribution)" \cite{slides6}. The colum-net model $H=(V,N)$ is defined as following: Let $I = \{0,1,\ldots,m-1\}$ and $J = \{0,1,\ldots,n-1\}$ then $I$ represents the rows and thus $V = I$ and $\forall i \in I$ we define a net $n_i \in N$ with  
			...
			\begin{figure}
					\begin{tikzpicture}
							...
							\node[vertex,below of=v1,label=above:\(v_2\)] (v2) {};
							...
							\begin{pgfonlayer}{background}
									\draw[edge,color=yellow] (v1) -- (v3);
									\draw[edge,opacity=1,color=green] (v3) -- (v4);
									\draw[edge,color=red] (v2) -- (v2);
									\draw[edge,line width=60pt ,color=blue] (v2) -- (v2);
							\end{pgfonlayer}

							\node[elabel,color=red,label=right:\(n_1\)]  (n1) at (-3,0) {};
							\node[elabel,below of=n1,color=green,label=right:\(n_2\)]  (n2) {};
							\node[elabel,below of=n2,color=orange,label=right:\(n_3\)]  (n3) {};
							\node[elabel,below of=n3,color=blue,label=right:\(n_4\)]  (n4) {};
							\node[elabel,below of=n4,color=yellow,label=right:\(n_4\)]  (n5) {};
					\end{tikzpicture}
					\centering
					\caption{The hypergraph of the $A_{5x4}$ matrix.}
					\label{fig:hypergraph}
			\end{figure}

		\item %TODO What cost function? How to partition?
